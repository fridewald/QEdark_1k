...
!
!
SUBROUTINE qedark_1k_f2( restartmode, &
     nksf, numval, numcond, &
  ...
  runff = .true. ! Set to false to not skip ff calculation --for debugging purposes
    ...
    CALL start_clock( ' qedark_f2_1k ')

    print *, "           -------             "
    print *, " calculation_mode == f2_1k"
    ...
    print *, "           -------             "
     ...
     IF (nspin .ne. 1) THEN
        CALL errore (' qedark_f2_1k ', 'Form factor calculation works only for spin-unpolarized systems!', 1)
     ENDIF


    IF ( nksf > nks .or. nksf < 0 ) &
         CALL errore( 'qedark_f2_1k',' nksf has a non-allowed value. Check input. ', ABS(ierr) )
    ...
    IF( numval>numvaltot .or. numcond>numcondtot .or. numval<1 .or. numcond<1 ) &
         CALL errore( 'qedark_f2_1k',' Check numval and numcond values in input ', ABS(ierr) )
    ...
    IF (noncolin .eqv. .false.) THEN
       ...
       IF( ierr /= 0 ) &
            CALL errore( 'qedark_f2_1k',' error allocating evcouter ', ABS(ierr) )
    ...
    IF( ierr /= 0 ) &
         CALL errore( 'qedark_f2_1k',' error allocating alligk ', ABS(ierr) )
    alligk(:,:)=0.0_DP ! when alligk=0, ig doesn't correspond to a G-vector in the set and should be disregarded

    WRITE(*,*), " "


    IF (num_er_bins > 999) CALL errore( 'qedark_f2_1k','Number of energy recoil bins cannot exceed 999 ', ABS(ierr) )
    IF (numqbins > 999) CALL errore( 'qedark_f2_1k','Number of momentum transfer bins cannot exceed 999 ', ABS(ierr) )
    ...
    IF (num_er_bins > 0) THEN
        ...
        IF( ierr /= 0 ) &
             CALL errore( 'qedark_f2_1k',' error allocating binedgesE ', ABS(ierr) )
        ...
        ! different binsize depending on bintype
    ...
    IF (numqbins > 0) THEN
       ...
       IF( ierr /= 0 ) &
            CALL errore( 'qedark_f2_1k',' error allocating binedgesq ', ABS(ierr) )
    ...
    IF( ierr /= 0 ) &
         CALL errore( 'qedark_f2_1k',' cannot allocate ctot ', ABS(ierr) )
    ctot(:,:) = 0.0_DP
    ...
    IF (runff) THEN
       ...
       ! Print to file
       WRITE (outfile, "(A2,I3.3,A4)") "K.", k_value, ".dat"
       print *, "Save data to ", outfile
    ...
    CALL stop_clock( ' qedark_f2_1k' )

END SUBROUTINE qedark_1k_f2
