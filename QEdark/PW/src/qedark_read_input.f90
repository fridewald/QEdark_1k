...
SUBROUTINE qedark_read_input(&
  ...
  INTEGER, INTENT(OUT) :: k_value                   ! k-value for integral
  !REAL(DP), intent(out) :: kk_SI                        ! normalization factor for eta
  ...
  deltav_SI(:) = 0.0_DP
  mx_NU(:) = 0.510998910_DP ! == electron rest mass
