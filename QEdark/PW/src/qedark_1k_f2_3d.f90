...
!
!
SUBROUTINE qedark_1k_f2_3d( restartmode, &
     nksf, numval, numcond, &
    ...
    !use input_parameters,               only: k_value
    ...
    CALL start_clock( ' qedark_1k_f2_3d ')

    print *, "           -------             "
    print *, " calculation_mode == f2_3d_1k"
    ...
    print *, "           -------             "
     ...
     IF (nspin .ne. 1) THEN
        CALL errore ('qedark_1k_f2_3d ', 'Form factor calculation works only for spin-unpolarized systems!', 1)
     ENDIF


    IF ( nksf > nks .or. nksf < 0 ) &
         CALL errore( 'qedark_1k_f2_3d ',' nksf has a non-allowed value. Check input. ', ABS(ierr) )
    ...
    IF( numval>numvaltot .or. numcond>numcondtot .or. numval<1 .or. numcond<1 ) &
         CALL errore( 'qedark_1k_f2_3d ',' Check numval and numcond values in input ', ABS(ierr) )
    ...
    IF (num_er_bins > 0) THEN
       ...
       CASE (1)
           ...
           er_binsize = ermax_NU/num_er_bins
       CASE (2)
           WRITE (*,*), "Energy bin size=", Ry2eV*er_binsize, "eV"
       CASE (3)
    ...
    IF (numqbins > 0) THEN
       ...
       IF( ierr /= 0 ) &
            CALL errore( 'qedark_1k_f2_3d ',' error allocating binedgesq ', ABS(ierr) )
    ...
    IF( ierr /= 0 ) &
         CALL errore( 'qedark_1k_f2_3d ',' cannot allocate ctot ', ABS(ierr) )
    ctot(:,:,:,:) = 0.0_DP
    ...
    CALL stop_clock( ' qedark_1k_f2_3d ' )

END SUBROUTINE qedark_1k_f2_3d
